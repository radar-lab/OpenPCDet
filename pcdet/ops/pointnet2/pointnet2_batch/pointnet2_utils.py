...
class GatherOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        B, npoint = idx.size()
        _, C, N = features.size()
        output = torch.empty(B, C, npoint, dtype=torch.float32, device='cuda')
    ...
    def backward(ctx, grad_out):
        ...
        grad_features = Variable(torch.zeros(B, C, N, dtype=torch.float32, device='cuda'))
...
class ThreeInterpolate(Function):
    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        grad_features = Variable(torch.zeros(B, c, m, dtype=torch.float32, device='cuda'))
        grad_out_data = grad_out.data.contiguous()
...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        _, C, N = features.size()
        output = torch.empty(B, C, nfeatures, nsample, dtype=torch.float32, device='cuda')
...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
        ...
        idx = torch.zeros(B, npoint, nsample, dtype=torch.int32, device='cuda')
